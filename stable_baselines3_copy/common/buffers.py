...
from stable_baselines3_copy.common.preprocessing import get_action_dim, get_obs_shape
from stable_baselines3_copy.common.type_aliases import ReplayBufferSamples, RolloutBufferSamples
from stable_baselines3_copy.common.vec_env import VecNormalize
...
class BaseBuffer(object):
    ...
    @staticmethod
    def swap_and_flatten_graph(l: list) -> list:
        result = []
        for partial_list in zip(*l):
...
class GNNRolloutBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        super(GNNRolloutBuffer, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        self.gae_lambda = gae_lambda
        self.gamma = gamma
        self.observations, self.actions, self.rewards, self.advantages = None, None, None, None
        self.returns, self.dones, self.values, self.log_probs = None, None, None, None
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        super(GNNRolloutBuffer, self).reset()
    ...
    def add(
            self, obs: list, action: np.ndarray, reward: np.ndarray, done: np.ndarray, value: th.Tensor,
            log_prob: th.Tensor
    ) -> None:
        """
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            for tensor in ["actions", "values", "log_probs", "advantages", "returns"]:
                self.__dict__[tensor] = self.swap_and_flatten(self.__dict__[tensor])
            self.observations = self.swap_and_flatten_graph(self.observations)
            self.generator_ready = True
